...
AI 辅助开发往往从一段自然语言想法开始，但在需求澄清、阶段计划、编码与验收之间，原始意图容易被改写、遗漏或散落在聊天记录中；需求变化后，也很难判断哪些实现和测试需要重新检查。更严重的是，审批、权限、保留期限等业务规则可能被 AI 的合理猜测悄悄当作已确认事实。

Project Map 将原始需求作为不可变来源保存，逐层拆解为可追溯的 Project → Epic → Feature → Story → Task，并把来源、确认过的决策、GSD 产物、代码和测试关联起来。它在计划或编码前执行确定性就绪检查：信息不足、关键决策未经授权或变更影响未复核时，流程会被明确阻断，而不是继续产生看似合理却不可验证的实现。

结果是：团队可以在 Codex 或 Claude Code 之间恢复同一项目上下文，知道当前只该推进哪个分支、为何可以开始实施，以及一次需求变更实际影响了什么。
